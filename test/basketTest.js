...
contract("BasketFacetTest", async (accounts) => {
  ...
  before(async () => {
    ...
    diamondCutFacet = new web3.eth.Contract(
      DiamondCutFacet.abi,
      diamond.address
    );
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await token.methods
        .approve(diamond.address, parseEther("100000000"))
  ...
  describe("Lock", async () => {
    ...
    it("Test setlock not allowed", async () => {
      await expect(
        basketFacet.methods.setLock(1).send({ from: accounts[1], gas: 1000000 })
      ).to.be.revertedWith("NOT_ALLOWED");
    });
    ...
    it("Initialize successful", async () => {
      ...
      for (var i = 0; i < tokens.length; i++) {
        await tokens[i].methods
          .transfer(diamond.address, parseEther("100"))
          .send({ from: web3.eth.defaultAccount, gas: 1000000 });
  ...
  describe("Joining and exiting", async () => {
    ...
    it("Join pool", async () => {
      for (var i = 0; i < tokens.length; i++) {
        ...
        balanceDiamondInternal = await basketFacet.methods
      ...
      tokensUser = await erc20Facet.methods
        .balanceOf(web3.eth.defaultAccount)
        .call();
